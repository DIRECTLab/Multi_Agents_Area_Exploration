...
for l in [2, 4, 6, 8, 10, 12, 14, 16]:
    for start in ["Edge", "Rand", "Top_Left"]:
        ...
        for k in [3,6,9,12,15,18,30]:
            searched_areas = []
            files = [f"./new_data50/multiple_method_search_data/start_scenario_{start}_Start_Position/goal_scenario_Edge_Start_Goal/bots-{l}_min-room-size{k}.0/seed-{j}_100explored_Exploration.csv" for j in range(50,100, 1)]
            result = []
            for file in files:
                if exists("" + file):
