...
class Agent(AStarPlanner):
    def __init__(self, 
            body_size, grid_size, lidar_range, 
        ...
        super().__init__( self.full_map)
        # self.plan = self.search(self.position,self.goal)
    ...
    def draw(self, screen):    
        ...
        pygame.draw.circle(screen, color= self.cur_color, 
        ...
        for i, angle in enumerate(np.arange(0, 2*np.pi, self.lidar_sweep_res)):
            ...
            for j, r in enumerate(ray_cast_samples):
                ...
                if sampled_point == False:# obstacle
                    ...
                    # ddraw the obstacle
                    pygame.draw.circle(screen, color= (255, 0, 0), center=(x*self.grid_size, y*self.grid_size), radius=self.grid_size//2)
                    break
                if r == max(ray_cast_samples):# frontier
                    self.built_map[y, x] = 2
                    pygame.draw.circle(screen, color= (0, 0, 255), center=(x*self.grid_size, y*self.grid_size), radius=self.grid_size//2)
                    break
                # free space
